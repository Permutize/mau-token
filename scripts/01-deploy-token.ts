...
async function main() {
  const ROOT_OWNER = process.env.ROOT_OWNER || ""

  if (!ROOT_OWNER) {
    throw new Error("ROOT_OWNER environment variable is not set")
  }
  
  const signer = (await locklift.keystore.getSigner("0"))!

  // Address of initial token supply recipient (write your own)
  const initialSupplyTo = new Address(ROOT_OWNER)
  // Address of token owner (write your own)
  const rootOwner = new Address(ROOT_OWNER)
  // Name of the token     
  const name = "Micro Venom Token"
  // Symbol of the token
  const symbol = "MVT"
